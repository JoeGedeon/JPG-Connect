...
// Routes archivist → ArchivistRoom, creative → KodexRoom, vera → VERARoom. OPS + KEL remain here.
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 5, background: "rgba(200,149,90,0.03)", border: "1px solid rgba(200,149,90,0.08)", borderLeft: "2px solid rgba(200,149,90,0.18)" }}>
        ...
        <div style={{ fontSize: "0.54rem", fontWeight: 600, color: "var(--fg-3)", lineHeight: 1.5, marginBottom: 10, letterSpacing: "0.01em" }}>Not to remember what happened.<br />To explain, defend, and prove it.</div>
        ...
        {["Reality enters through witnesses.","The ledger is the record. It belongs to the organization.","K.E.L. defends. VERA explains. The tiers prove.","The ledger cannot invent a record. It can only receive one."].map((line, i) => (
          <div key={i} style={{ fontSize: "0.46rem", color: "var(--fg-4)", lineHeight: 1.85, letterSpacing: "0.02em" }}>{line}</div>
        ))}
        ...
        {total > 0 && (
          <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 5, background: `${mis.color}08`, border: `1px solid ${mis.color}20`, borderLeft: `2px solid ${mis.color}50` }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ fontSize: "1.1rem", fontWeight: 800, color: mis.color, lineHeight: 1, letterSpacing: "-0.02em" }}>{mis.score}</div>
            </div>
            <div style={{ display: "flex", gap: 4, marginBottom: 7 }}>
              {[{label:"reliability",value:mis.components.reliability},{label:"attribution",value:mis.components.attribution},{label:"context",value:mis.components.context},{label:"recency",value:mis.components.recency}].map(({label,value}) => (
                <div key={label} style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.3rem", fontFamily: "monospace", color: "var(--fg-4)", textAlign: "center", letterSpacing: "0.06em" }}>{label.slice(0,4)}</div>
                </div>
              ))}
            </div>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: "0.4rem", fontFamily: "monospace", color: "var(--fg-4)", letterSpacing: "0.06em" }}>{stats.totalEvents} events total</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        <div style={{ padding: "10px 12px", borderRadius: 6, background: "var(--bg-card)", border: `1px solid ${decisionReady ? "#7bc85a30" : "var(--border)"}`, borderLeft: `2px solid ${decisionReady ? "#7bc85a" : "#1d1d38"}` }}>
          ...
          <div style={{ fontSize: "0.46rem", color: "var(--fg-4)", lineHeight: 1.5, marginBottom: 3 }}>{decisionReady ? "Active — approver patterns visible" : "Which approvers make the best calls?"}</div>
          <div style={{ fontSize: "0.44rem", fontFamily: "monospace", color: decisionReady ? "#7bc85a" : "var(--fg-4)" }}>{decisionReady ? "✓ threshold met" : `${stats.attributedCount} / ${stats.attributedThreshold} attributed events`}</div>
          {!decisionReady && <ProgressBar pct={decisionPct} color="#7bc85a60" />}
        </div>
        <div style={{ padding: "10px 12px", borderRadius: 6, background: "var(--bg-card)", border: `1px solid ${patternReady ? "#5a9bc830" : "var(--border)"}`, borderLeft: `2px solid ${patternReady ? "#5a9bc8" : "#1d1d38"}` }}>
          ...
          <div style={{ fontSize: "0.46rem", color: "var(--fg-4)", lineHeight: 1.5, marginBottom: 3 }}>{patternReady ? "Active — recurring patterns detectable" : "What repeats across event types?"}</div>
          <div style={{ fontSize: "0.44rem", fontFamily: "monospace", color: patternReady ? "#5a9bc8" : "var(--fg-4)" }}>{patternReady ? "✓ threshold met" : `${stats.maxTypeDensity} / ${stats.patternThreshold} densest type`}</div>
        ...
        <div style={{ padding: "10px 12px", borderRadius: 6, background: "var(--bg-card)", border: `1px solid ${gapsClean ? "rgba(76,217,100,0.15)" : "rgba(255,107,107,0.15)"}`, borderLeft: `2px solid ${gapsClean ? "rgba(76,217,100,0.5)" : "rgba(255,107,107,0.5)"}` }}>
          ...
          {!gapsClean && stats.recentGaps.length > 0 && (
            <div style={{ marginTop: 4 }}>{stats.recentGaps.slice(0,2).map(g => (<div key={g.id} style={{ fontSize: "0.4rem", fontFamily: "monospace", color: "#ff6b6b80", lineHeight: 1.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.id}: {g.description?.slice(0,40) || g.type}…</div>))}</div>
          )}
        </div>
      ...
      <div style={{ marginTop: 7, fontSize: "0.4rem", fontFamily: "monospace", color: "var(--fg-4)", opacity: 0.45, letterSpacing: "0.04em" }}>The history is the intelligence. Each attributed event moves these thresholds.</div>
    </div>
  )
}
...
function IngestionMonitor({ lc }) {
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      {lastIn && (
        <div style={{ marginBottom: 8, padding: "7px 11px", borderRadius: 5, background: "rgba(90,155,200,0.06)", border: "1px solid rgba(90,155,200,0.2)", fontSize: "0.52rem", fontFamily: "monospace", color: "#5a9bc8", letterSpacing: "0.06em" }}>↳ {lastIn} — testimony recorded</div>
      )}
        ...
        return (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 6, padding: "9px 12px", borderRadius: 6, background: done ? "rgba(90,155,200,0.04)" : "var(--bg-card)", border: `1px solid ${done ? "rgba(90,155,200,0.2)" : "var(--border)"}`, transition: "all 0.2s" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: "0.42rem", fontFamily: "monospace", color: "var(--fg-4)", letterSpacing: "0.06em" }}>{demo.ff_job_id && `${demo.ff_job_id} · `}{new Date(demo.ff_timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}{demo.ff_amount && ` · $${demo.ff_amount.toLocaleString()}`}</div>
            </div>
            <button onClick={() => !done && simulate(demo)} disabled={done} style={{ padding: "5px 10px", borderRadius: 4, border: `1px solid ${done ? "rgba(90,155,200,0.2)" : "#5a9bc840"}`, background: done ? "rgba(90,155,200,0.06)" : "transparent", color: done ? "#5a9bc8" : "var(--fg-4)", fontSize: "0.46rem", fontFamily: "monospace", letterSpacing: "0.08em", cursor: done ? "default" : "pointer", flexShrink: 0, transition: "all 0.15s" }}>{done ? "✓ ingested" : "ingest →"}</button>
          </div>
        )
      })}
      <div style={{ marginTop: 6, fontSize: "0.4rem", fontFamily: "monospace", color: "var(--fg-4)", opacity: 0.5, letterSpacing: "0.06em" }}>all testimony enters the ledger · the record belongs to the organization (JPG-019)</div>
    </div>
  )
}
    ...
    <div style={{ paddingTop: 40, paddingBottom: 20 }}>
      ...
      {debt.pendingReviews > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12, padding: "10px 13px", borderRadius: 7, background: "rgba(232,168,124,0.06)", border: "1px solid rgba(232,168,124,0.22)" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.52rem", color: "var(--fg-4)", fontFamily: "monospace", marginTop: 2 }}>new declarations awaiting comparison with foundational doctrine</div>
          </div>
        </div>
      )}
      {opsTensions.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22, padding: "10px 13px", borderRadius: 7, background: "#c87dff0a", border: "1px solid #c87dff30" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "0.52rem", color: "var(--fg-4)", fontFamily: "monospace", marginTop: 2 }}>{opsTensions.length === 1 ? "1 open tension" : `${opsTensions.length} open tensions`} affecting operations</div>
          </div>
        </div>
      )}
      {tasks.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 28 }}>
          {[{label:"total",count:tasks.length,color:"var(--fg-3)"},{label:"pending",count:pending,color:"#ff9f43"},{label:"active",count:active,color:lc.color}].map(({label,count,color}) => (
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {(STARTERS.ops || []).map((s, i) => (
          <div key={i} onClick={() => onSend(s)} style={{ padding: "7px 13px", border: "1px solid var(--border)", borderRadius: 6, fontSize: "0.74rem", color: "var(--fg-3)", cursor: "pointer", background: "var(--bg-card)", transition: "all 0.15s" }}
  ...
  return (
    <div style={{ paddingTop: 32, paddingBottom: 20 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          {buckets.map(({ label, tasks: bt, color, note }) => (
            <div key={label} style={{ marginBottom: 8, padding: "10px 13px", borderRadius: 7, background: "var(--bg-card)", border: "1px solid var(--border)", borderLeft: `2px solid ${bt.length > 0 ? color : "var(--border)"}` }}>
              ...
              {note && bt.length > 0 && <div style={{ fontSize: "0.54rem", color: "#c87dff80", fontFamily: "monospace" }}>{note}</div>}
            </div>
          ))}
        </div>
      ) : (
      ...
      {KEL_STARTERS.map((s, i) => (
        <div key={i} onClick={() => onSend(s)} style={{ marginBottom: 6, padding: "9px 13px", border: "1px solid var(--border)", borderRadius: 6, fontSize: "0.74rem", color: "var(--fg-3)", cursor: "pointer", background: "var(--bg-card)", transition: "all 0.15s" }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = lc.color; e.currentTarget.style.color = "var(--fg)"; e.currentTarget.style.background = lc.dim }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.color = "var(--fg-3)"; e.currentTarget.style.background = "var(--bg-card)" }}>
          {s}
        </div>
